...
uint32_t CAN_ID2Ptr[4];//预先定义四个小米电机id与对应指针的映射
static uint8_t num_id2ptr =0;
...
{
	if(num_id2ptr >= 4) return;  // 添加边界检查
	// 检查ID是否重复
	for(int i = 0; i < num_id2ptr; i++) {
			if(CAN_ID2Ptr[i] == Can_Id) return;  // 防止重复ID
	}
	...
	CAN_ID2Ptr[num_id2ptr]=Can_Id;// 记录ID映射
...
/**
  * @brief          速度模式下设置目标速度
  * @param[in]      Motor:  电机结构体
  * @param[in]      target_speed:  目标速度 [rad/s]
  * @retval         none
  */
void set_target_speed(MI_Motor *Motor, float target_speed)
{
    Set_Motor_Parameter(Motor, Spd_Ref, target_speed, 'f');
}

/**
  * @brief          位置模式下设置目标位置
  * @param[in]      Motor:  电机结构体
  * @param[in]      target_position:  目标位置 [rad]
  * @retval         none
  */
void set_target_position(MI_Motor *Motor, float target_position)
{
    Set_Motor_Parameter(Motor, Loc_Ref, target_position, 'f');
}

/**
  * @brief          电流模式下设置目标电流
  * @param[in]      Motor:  电机结构体
  * @param[in]      target_current:  目标电流 [A]
  * @retval         none
  */
void set_target_current(MI_Motor *Motor, float target_current)
{
    Set_Motor_Parameter(Motor, Iq_Ref, target_current, 'f');
}
/*****************************回调函数 负责接回传信息 可转移至别处*****************************/
/**
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    if(hcan->Instance == CAN1) {
        ...
        // 在ID映射表中查找对应的电机索引
        for(int i = 0; i < num_id2ptr; i++) {
            if(CAN_ID2Ptr[i] == Motor_Can_ID) {
                if(rxMsg.ExtId >> 24 != 0) {
                    Motor_Data_Handler(&mi_motor[i], rx_data, rxMsg.ExtId);
                } else {
                    mi_motor[i].MCU_ID = rx_data[0];
                }
                break;
            }
        }
